...
let
  ...
  installScript = pkgs.writeScriptBin "install-yek" (builtins.readFile (pkgs.replaceVars ./install-yek-shim.sh {
    bash = pkgs.bash;
  ...
  yekWrapper = pkgs.writeScriptBin "yek" (builtins.readFile (pkgs.replaceVars ./yek-shim.sh {
    bash = pkgs.bash;
