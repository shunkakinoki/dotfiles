...
describe("settings", function()
	...
	describe("visible whitespace", function()
		it("should enable list mode", function()
			assert.is_true(vim.opt.list:get())
		end)

		it("should configure tab listchar", function()
			local lc = vim.opt.listchars:get()
			assert.is_not_nil(lc.tab)
		end)

		it("should configure trail listchar", function()
			local lc = vim.opt.listchars:get()
			assert.is_not_nil(lc.trail)
		end)

		it("should configure extends listchar", function()
			local lc = vim.opt.listchars:get()
			assert.is_not_nil(lc.extends)
		end)

		it("should configure precedes listchar", function()
			local lc = vim.opt.listchars:get()
			assert.is_not_nil(lc.precedes)
		end)

		it("should configure nbsp listchar", function()
			local lc = vim.opt.listchars:get()
	...
	describe("text wrapping", function()
		...
		it("should set textwidth to 0", function()
			assert.equals(0, vim.opt.textwidth:get())
		end)
	end)
	...
	describe("misc options", function()
		...
		it("should enable ruler", function()
			assert.is_true(vim.opt.ruler:get())
		end)

		it("should enable wildmenu", function()
			assert.is_true(vim.opt.wildmenu:get())
		end)

		it("should set laststatus to 2", function()
			assert.equals(2, vim.opt.laststatus:get())
		end)

		it("should configure backspace with indent, eol, start", function()
			local bs = vim.opt.backspace:get()
			assert.is_true(vim.tbl_contains(bs, "indent"))
			assert.is_true(vim.tbl_contains(bs, "eol"))
			assert.is_true(vim.tbl_contains(bs, "start"))
		end)
	end)

	describe("diagnostic config", function()
		it("should have virtual_text configured", function()
			local config = vim.diagnostic.config()
			assert.is_not_nil(config.virtual_text)
		end)

		it("should have signs enabled", function()
			local config = vim.diagnostic.config()
			assert.is_true(config.signs ~= false)
		end)

		it("should have underline enabled", function()
			local config = vim.diagnostic.config()
			assert.is_true(config.underline ~= false)
		end)

		it("should not update diagnostics in insert mode", function()
			local config = vim.diagnostic.config()
			assert.is_false(config.update_in_insert)
		end)

		it("should sort by severity", function()
			local config = vim.diagnostic.config()
			assert.is_true(config.severity_sort ~= false)
		end)
	end)

	describe("fugitive", function()
		it("should disable legacy commands", function()
			assert.equals(0, vim.g.fugitive_legacy_commands)
		end)
	end)
end)
