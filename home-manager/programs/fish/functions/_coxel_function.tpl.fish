function _coxel_function --description "Run Codex with a free-form prompt using the local Qwen model"
  ...
  if test (count $argv) -gt 0; and contains -- "$argv[1]" --resume -r --continue -c
    ...
    codex $bypass_flag --oss --local-provider lmstudio --model '__QWEN_LOCAL__' --full-auto -c model_reasoning_effort=minimal
  else
    set -l prompt (string join " " -- $argv)
    codex $bypass_flag exec --oss --local-provider lmstudio --model '__QWEN_LOCAL__' --full-auto -c model_reasoning_effort=minimal -- "$prompt"
  end
end
