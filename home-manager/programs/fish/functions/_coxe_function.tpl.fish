function _coxe_function --description "Run Codex with a free-form prompt"
  ...
  if test (count $argv) -gt 0; and contains -- "$argv[1]" --resume -r --continue -c
    ...
    codex $bypass_flag --model '__GPT__' --full-auto -c model_reasoning_summary_format=experimental
  else
    set -l prompt (string join " " -- $argv)
    codex $bypass_flag exec --model '__GPT__' --full-auto -c model_reasoning_summary_format=experimental -- "$prompt"
  end
end
