function _coxel_function --description "Run Codex with a free-form prompt using the local Qwen model"
  ...
  if test (count $argv) -gt 0; and contains -- "$argv[1]" --resume -r --continue -c
    ...
    codex --dangerously-bypass-approvals-and-sandbox --oss --local-provider lmstudio --model 'qwen3.5-0.8b-optiq' -c model_reasoning_effort=minimal
  else
    set -l prompt (string join " " -- $argv)
    codex --dangerously-bypass-approvals-and-sandbox exec --oss --local-provider lmstudio --model 'qwen3.5-0.8b-optiq' -c model_reasoning_effort=minimal -- "$prompt"
  end
end
