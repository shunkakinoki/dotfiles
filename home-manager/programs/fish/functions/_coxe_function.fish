function _coxe_function --description "Run Codex with a free-form prompt"
  # Run Codex with a free-form prompt (spaces allowed)
  # Usage: cxe [<prompt words...>]

  if test (count $argv) -gt 0; and contains -- "$argv[1]" --resume -r --continue -c
    _caam_exec_function codex --dangerously-bypass-approvals-and-sandbox resume $argv[2]
  else if test (count $argv) -eq 0
    _caam_exec_function codex --dangerously-bypass-approvals-and-sandbox --model 'gpt-6-sol' -c model_reasoning_summary_format=experimental
  else
    set -l prompt (string join " " -- $argv)
    _caam_exec_function codex --dangerously-bypass-approvals-and-sandbox exec --model 'gpt-6-sol' -c model_reasoning_summary_format=experimental -- "$prompt"
  end
end
