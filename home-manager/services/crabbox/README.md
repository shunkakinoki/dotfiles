...
After the Cloudflare stack creates the Access application, put its audience tag in
Kyber's `~/dotfiles/.env` as `CRABBOX_ACCESS_AUD`. The Access team domain defaults to
...
CLI clients also need the dedicated Access service token generated by the Cloudflare
stack. Store its client ID and secret in the client's private Crabbox config under
