...
When run bash -c "jq -e '.model == null and .retry_on_errors == [401,404,429,500,502,503,504] and .retryable_error_patterns == [\"unknown provider for model\"] and .max_fallback_attempts == 5 and .fallback_models == [\"shunkakinoki/free\",\"cliproxyapi/free\"]' config/opencode/opencode-fallback.jsonc >/dev/null"
The status should be success
End
...
When run bash -c "sed -n '/name: \"openrouter\"/,/name: \"z-ai\"/p' config/cliproxyapi/config.template.yaml | grep -q 'name: \"deepseek/deepseek-v4.1-flash\"' && sed -n '/name: \"opencode\"/,/name: \"verboo\"/p' config/cliproxyapi/config.template.yaml | grep -q 'alias: \"deepseek-v4.1-flash\"'"
The status should be success
End
