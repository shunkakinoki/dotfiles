...
When run bash -c "jq -e '.agents.ownership == \"explicit\" and (.agents | has(\"list\") | not) and (.agents.entries | any(.default == true) | not) and .agents.defaults.systemAgent.agentId == \"main\" and .agents.defaults.experimental.localModelLean == true and .tools.codeMode == true and .tools.toolSearch == true and .tools.loopDetection.enabled == true and .tools.swarm == {enabled: true, maxConcurrent: 8, maxChildrenPerGroup: 50, maxTotalPerGroup: 200, waitTimeoutSecondsMax: 600, defaultAgentId: \"\"} and .gateway.cliAgents.enabled == true and .logging.audit.messages == \"direct\" and .desktop.host.enabled == true and .cloudWorkers.desktop == true' '$PWD/config/openclaw/openclaw.tpl.json' >/dev/null"
The status should be success
End
...
When run bash -c "gateway=\$(sed -n '/systemd.user.services.openclaw-gateway =/,/Install = {/p' '$PWD/home-manager/services/openclaw/default.nix'); grep -qF 'ExecStartPre = \"-\${tmpCleanup}/bin/openclaw-gateway-tmp-cleanup\";' <<<\"\$gateway\" && grep -qF '\"TMPDIR=/dev/shm/openclaw-gateway\"' <<<\"\$gateway\""
The status should be success
End
