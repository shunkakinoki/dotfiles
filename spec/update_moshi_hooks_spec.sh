...
When run bash -c 'HOME="$REGEN_HOME" bash "$REGEN_ACTIVATE_SCRIPT" "$REGEN_CONFIG_TOML" "$REGEN_HOOKS_JSON" "$REGEN_DESKTOP_SETTINGS" jq "$REGEN_SYNC" "$REGEN_PROFILES_DIR" "$REGEN_PRINTER" >/dev/null && jq -e --arg unrelated "$REGEN_UNRELATED" --arg unrelatedPrefix "$REGEN_UNRELATED_PREFIX" --arg privateSource "$REGEN_SOURCE" '"'"'(.hooks.SessionStart[0].hooks += [{"command":$unrelated,"type":"command"},{"command":$unrelatedPrefix,"type":"command"}])'"'"' "$REGEN_HOME/.codex/hooks.json" >"$REGEN_ACTIVATED_INPUT" && HOME="$REGEN_HOME" PATH="$REGEN_BIN:$PATH" GENERATED_ROOT="$REGEN_OUTPUT" bash "$REGEN_SCRIPT" >/dev/null && jq -e --arg marker "$REGEN_MARKER" --arg unrelated "$REGEN_UNRELATED" --arg unrelatedPrefix "$REGEN_UNRELATED_PREFIX" --arg privateSource "$REGEN_SOURCE" '"'"'([.hooks.SessionStart[]?.hooks[]?.command] | map(select(. == $marker)) | length == 1) and ([.hooks.UserPromptSubmit[]?.hooks[]?.command] | map(select(. == $marker)) | length == 1) and ([.hooks[][]?.hooks[]?.command? | strings] | all(contains($privateSource) | not)) and ([.hooks.SessionStart[]?.hooks[]?.command, .hooks.UserPromptSubmit[]?.hooks[]?.command] | index($unrelated)) and ([.hooks.SessionStart[]?.hooks[]?.command, .hooks.UserPromptSubmit[]?.hooks[]?.command] | index($unrelatedPrefix)) and ([.hooks.SessionStart[]?.hooks[]?.command, .hooks.UserPromptSubmit[]?.hooks[]?.command] | index("moshi-hook codex-hook")) and ([.hooks.SessionStart[]?.hooks[]?.command, .hooks.UserPromptSubmit[]?.hooks[]?.command] | index("bd codex-hook UserPromptSubmit"))'"'"' "$REGEN_OUTPUT/codex/hooks.json"'
The status should be success
