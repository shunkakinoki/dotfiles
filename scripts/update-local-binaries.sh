...
find_build_dir() {
  ...
  while true; do
    if [ -f "$dir/Makefile" ] || [ -f "$dir/Cargo.toml" ] || [ -f "$dir/go.mod" ] || [ -f "$dir/mix.exs" ] || [ -f "$dir/pyproject.toml" ] || [ -f "$dir/package.json" ]; then
      echo "$dir"
      return 0
    fi
    [ "$dir" = "$repo_dir" ] && break
...
build_repo() {
  ...
  if [ -n "$_js_install_cmd" ] && [[ ",$flags," != *,node-build,* ]]; then
...
WRAPPER
    ...
    return 0
  elif [ -f "$build_dir/package.json" ] && [[ ",$flags," == *,node-build,* ]]; then
    if ! command -v npm >/dev/null 2>&1; then
      log_error "  npm not found; cannot build Node coordinator"
      return 1
    fi
    if ! (cd "$build_dir" && npm ci 2>&1 && npm run build:node 2>&1); then
      return 1
    fi
    if [ -n "$binary_path" ]; then
      local expanded_bin="${binary_path/#\~/$HOME}"
      local server_entry="$build_dir/dist-node/server.mjs"
      if [ ! -f "$server_entry" ]; then
        log_error "  Node build did not produce $server_entry"
        return 1
      fi
      mkdir -p "$(dirname "$expanded_bin")"
      cat >"$expanded_bin" <<WRAPPER
#!/usr/bin/env bash
# Auto-generated by scripts/update-local-binaries.sh
set -euo pipefail
...
WRAPPER
      chmod +x "$expanded_bin"
      if [[ $expanded_bin == "$repo_dir"/* ]]; then
        local exclude_entry="/${expanded_bin#"$repo_dir"/}"
        if ! grep -qxF "$exclude_entry" "$repo_dir/.git/info/exclude" 2>/dev/null; then
          echo "$exclude_entry" >>"$repo_dir/.git/info/exclude"
        fi
      fi
      log_step "  Wrote Node wrapper: $(basename "$expanded_bin") -> $server_entry"
    fi
    return 0
  else
    log_warn "  No supported build configuration found, skipping build"
    return 0
  fi
}
