...
if [ -z "$CLIPROXY_API_KEY" ]; then
  echo "Warning: CLIPROXY_API_KEY not found; T3 Code CLIProxy instances stay unauthenticated" >&2
fi

# The base URL is host-only: Claude Code appends /v1/messages to it, so a
# trailing /v1 would produce /v1/v1/messages. Remote by default; a host that runs
# its own proxy overrides it.
